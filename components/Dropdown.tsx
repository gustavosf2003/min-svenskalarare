  ...
  return (
    <div className="relative w-40">
      <Listbox value={selectedItem} onChange={setSelectedItem}>
        ...
        <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-[#2F2F2F] py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm z-20">
          {items.map((item) => (
            <Listbox.Option
              className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-basePrimary"
              key={item.icon}
              value={item}
            >
